...
class GameProcess:
    def __init__(self):
        global keys, TILE, imgBrick, window, sndShot, sndDestroy, DIRECTS, WIDTH, HEIGHT, imgBangs, bullets, objects
        pygame.init()
        ...
        TILE = 32
        fontMain = pygame.font.Font('fonts/better-vcr_0.ttf', 50)
        fontSelect = pygame.font.Font('fonts/better-vcr_0.ttf', 60)
        items = ['Exit']
        ...
        while play:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    play = False
                if event.type == pygame.KEYDOWN and isWin:
            ...
            if t == 1 and isWin:
                for i in range(len(items)):
                    if i == select and timer % 30 < 15:
                        text = fontSelect.render(items[i], 0, 'white')
                    else:
                        text = fontMain.render(items[i], 0, 'gray')
                    rect = text.get_rect(center=(WIDTH // 2, 400 + 70 * i))
                    window.blit(text, rect)
                f = open('results.txt', encoding='utf-8')
                frst, scnd = list(map(int, f.read().split()))
                f.close()
                if playerWin.color == 'gray' and not endGame:
                    frst += 1
                    endGame = True
                elif playerWin.color == 'red' and not endGame:
                    scnd += 1
                    endGame = True
                f = open('results.txt', 'w')
                f.write(f'{frst} {scnd}')
                f.close()
                text = fontItem.render(f'g:{frst} r:{scnd}', 1, 'white')
